...
from gibson2.robots.custombot_robot import Custombot
from gibson2.simulator import Simulator
from gibson2.scenes.empty_scene import EmptyScene
from gibson2.scenes.simple_scene import SimpleScene
from gibson2.scenes.wall_scene import WallScene
from gibson2.scenes.wall_large_scene import WallLargeScene
...
class BaseEnv(gym.Env):
    ...
    def load(self):
        ...
        if self.config['scene'] == 'empty':
        ...
        elif self.config['scene'] == 'simple':  # simple scene with 2 walls
            scene = SimpleScene()
            self.simulator.import_scene(
                scene, load_texture=self.config.get('load_texture', True))
            ...
            self.simulator.import_scene(
                scene, load_texture=self.config.get('load_texture', True))
        elif self.config['scene'] == 'walls':    # wall scenes
            scene = WallScene(
                self.config['scene_id'],
                waypoint_resolution=self.config.get(
                    'waypoint_resolution', 0.2),
                num_waypoints=self.config.get('num_waypoints', 6)+1,
                build_graph=self.config.get('build_graph', False),
                trav_map_resolution=self.config.get(
                    'trav_map_resolution', 0.1),
                trav_map_type=self.config.get('trav_map_type', 'no_obj'),
            ...
            self.simulator.import_scene(
                scene, load_texture=self.config.get('load_texture', True))
        elif self.config['scene'] == 'walls_large':    # large wall scenes
            scene = WallLargeScene(
                self.config['scene_id'],
                waypoint_resolution=self.config.get(
                    'waypoint_resolution', 0.2),
                num_waypoints=self.config.get('num_waypoints', 6)+1,
        ...
        elif self.config['robot'] == 'Custombot':
            robot = Custombot(self.config)
        else:
            raise Exception(
