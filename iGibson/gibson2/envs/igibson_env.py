...
from gibson2.tasks.social_nav_fixed_task import SocialNavFixedTask
...
class iGibsonEnv(BaseEnv):
    ...
    def load_task_setup(self):
        ...
        elif self.config['task'] == 'social_nav_fixed':
            self.task = SocialNavFixedTask(self)
        else:
            self.task = None
    ...
    def get_state(self, collision_links=[]):
        ...
        if 'goal' in self.output:
            state['goal'] = self.task.get_goal_obs(self)
        if 'waypoints' in self.output:
            state['waypoints'] = self.task.get_waypoints_obs(self)
        if 'pedestrians' in self.output:
            state['pedestrians'] = self.task.get_pedestrians_obs(self)
