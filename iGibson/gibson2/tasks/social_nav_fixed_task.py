...
class SocialNavFixedTask(PointNavFixedTask):
    """
    Social Navigation Fixed Task
    The goal is to navigate to a fixed point in the presence of pedestrians
    """
    def __init__(self, env):
        super(SocialNavFixedTask, self).__init__(env)
        ...
        #self.target_pos = np.array(self.config.get('target_pos', [4, 1, 0]))
        self.ped_initial_pos = np.array(self.config.get('ped_initial_pos', [-4., 0., 0.]))
        self.ped_initial_orn = np.array([0, 0, 0])
        tmp = np.array(self.config.get('ped_target_pos', [4., 0., 0.]))
        self.ped_target_pos = np.array([tmp, self.ped_initial_pos])
        self.ped_target_id = 0
        """
    ...
    def reset_pedestrians(self, env):
        ...
        for ped_id, (ped, orca_ped) in enumerate(zip(self.pedestrians, self.orca_pedestrians)):
            if self.offline_eval:
                ...
                initial_pos = self.ped_initial_pos
    ...
    def reset_agent(self, env):
        ...
        :param env: environment instance
        """
        super(SocialNavFixedTask, self).reset_agent(env)
        if self.offline_eval:
    ...
    def sample_new_target_pos(self, env, initial_pos, ped_id=None):
        ...
        while True:
            if self.offline_eval:
                ...
                target_pos = self.ped_target_pos[self.ped_target_id]
                self.ped_target_id = 1 - self.ped_target_id
                #target_pos = np.array(self.ped_initial_pos)
    ...
    def step(self, env):
        ...
        :param env: environment instance
        """
        super(SocialNavFixedTask, self).step(env)
        ...
        for i, (ped, orca_pred, waypoints) in \
            ...
            if next_peds_stop_flag[i] is True:
                ...
                # advance pybullet pedestrian to the current time step
                self.num_steps_stop[i] = 0
    ...
    def get_termination(self, env, collision_links=[], action=None, info={}):
        ...
        """
        done, info = super(SocialNavFixedTask, self).get_termination(
    ...
    def get_task_obs(self, env):
        ...
        """
        task_obs = super(SocialNavFixedTask, self).get_task_obs(env)
        for ped in self.pedestrians:
            ped_pos = ped.get_position()
        ped_pos = self.global_to_local(env, ped_pos)[:2]
        ped_pos = np.array(cartesian_to_polar(ped_pos[0], ped_pos[1]))
        waypoints = self.update_waypoints(env)
        task_obs = np.append(task_obs, ped_pos)
