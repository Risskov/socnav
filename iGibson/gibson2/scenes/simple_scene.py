...
class SimpleScene(Scene):
    ...
    def __init__(self):
        super(SimpleScene, self).__init__()
    ...
    def load(self):
        ...
        wall2 = p.createMultiBody(baseCollisionShapeIndex=wall_coll,
                                  basePosition=[0, -2.2, 0])
        #p.changeDynamics(wall1, -1, lateralFriction=0.1, restitution=0.1)
        print("Shape data ", p.getVisualShapeData(wall1))
        print("Floor data ", p.getVisualShapeData(self.floor_body_ids[0]))
        print("Mesh data ", p.getMeshData(wall1))
        self.floor_body_ids.append(wall1)
    ...
    def get_shortest_path(self, floor, source_world, target_world, entire_path=False):
        ...
        """
        logging.warning(
            'WARNING: trying to compute the shortest path in SimpleScene (assuming empty space)')
